...
return new class {
    ...
    public function getBoxButton(string $link, string $iconName, string $label, array $options = []): string
    {
        ...
        $plugin = Core::$pluginManager->getPlugins()->getExtend('admin-plus');
    ...
    public function __invoke(array $args)
    {
        ...
        if ($config['show_service_buttons']) {
        ...
        if ($_admin->currentModule === 'content-editgallery' && !$new) {
            ...
            if (User::hasPrivilege('adminsettings')) {
                $buttons['image_settings'] = $this->getBoxButton(
                    ...
                    'settings',
                    _lang('admin-plus.btn.gallery.settings'),
                    ['css_class' => ['button', 'block', 'bigger']]
                );
            }
        }
        ...
        // event
        $editscript_setting_extra .= Extend::buffer('admin-plus.buttonbox.before', [
            'admin' => $_admin,
            'new' => $new,
            'buttons' => &$buttons,
        ]);
        // render buttonbox
        if (!empty($buttons)) {
            $editscript_setting_extra .= '<div class="adminplus-buttonbox">' . implode('', $buttons) . '</div>';
        }
        //event
        $editscript_setting_extra .= Extend::buffer('admin-plus.buttonbox.after', [
            'admin' => $_admin,
        ]);
    }
};
